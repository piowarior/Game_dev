...
# =====================================================================
# NAVIGATION & EXIT LOGIC (FIXED)
# =====================================================================

# PANGGIL INI SAAT TOMBOL EXIT DI MENU PAUSE DITEKAN
func _on_exit_button_pressed():
    # Sangat penting: matikan pause sebelum pindah scene
...
func _start_quake():
    ...
    if sfx_warning: sfx_warning.stop()
    if bgm_player:
        create_tween().tween_property(bgm_player, "volume_db", bgm_quake_volume, 0.6)
    if sfx_quake:
        sfx_quake.play()

    create_tween().tween_property(camera, "zoom", base_camera_zoom, 0.4)
...
func _end_quake():
    ...
    if warning_blink_tween: warning_blink_tween.kill()
    ...
    if bgm_player:
        create_tween().tween_property(bgm_player, "volume_db", bgm_normal_volume, 1.0)
