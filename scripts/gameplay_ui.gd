...
# ===============================
func _ready():
    # Agar UI tetap jalan saat game di-pause
    self.process_mode = Node.PROCESS_MODE_ALWAYS
    
    mission_panel.visible = false
    pause_panel.visible = false

    # --- LOGIKA KLIK BACKPACK ---
    if is_instance_valid(backpack_box):
        ...
        # Otomatis set semua anak (Sprite2D, TextureRect, Label) agar mengabaikan klik
        # Ini supaya klik kita langsung mengenai 'Backpack' (induknya)
        for child in backpack_box.get_children():
            if child is Control:
                child.mouse_filter = Control.MOUSE_FILTER_IGNORE
            elif child is CanvasItem: # Untuk Sprite2D/TextureRect
                # Secara default Sprite2D tidak memblokir mouse, tapi kita pastikan area terpantau
                pass
        
        # Hubungkan sinyal jika belum terhubung
        if not backpack_box.gui_input.is_connected(_on_backpack_gui_input):
            backpack_box.gui_input.connect(_on_backpack_gui_input)
    
    # Hubungkan tombol Misi
...
func _on_backpack_gui_input(event):
    ...
    if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
        if event.pressed:
            # Jika ini muncul di terminal, berarti area klik sudah benar
            print("DEBUG: Tombol Backpack diklik!") 
            backpack_box.accept_event() # Hentikan event agar tidak diproses hal lain
            _toggle_backpack_ui()
...
func _toggle_backpack_ui():
    # Cari BackpackUI yang berada satu level di bawah root scene bersama GameplayUi
    var backpack_ui = get_parent().get_node_or_null("BackpackUI")
    
    if backpack_ui:
        # Tutup menu lain jika sedang terbuka
        if pause_panel.visible or mission_panel.visible:
            _close_all_ui()
        
        # Panggil fungsi toggle yang sudah kita buat di backpack_ui.gd
...
func _process(delta):
    ...
    if GameState.time_left <= 0: return
...
func _update_hud():
    ...
    if GameState.current_mission.has("total_victim"):
        label_victim.text = "%d / %d" % [GameState.victim_saved, GameState.current_mission.total_victim]
    else:
